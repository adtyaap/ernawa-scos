...
- **Sisa dari advisor Supabase (belum dikerjakan, bukan kritis)**: (a) fungsi trigger SECURITY DEFINER (`fn_*_ledger`, `fn_user_sites_audit`, dst.) masih EXECUTE-able oleh anon/authenticated lewat `/rest/v1/rpc` — tidak bisa dipanggil langsung (trigger-only) tapi sebaiknya `REVOKE EXECUTE`; (b) 12 fungsi SECURITY INVOKER belum mengunci `search_path`; (c) "Leaked password protection" belum aktif di Supabase Auth (setelan dashboard) — relevan karena akun baru memakai katasandi sementara. (d) `rls_auto_enable()` bukan buatan migration proyek ini — periksa asalnya sebelum mengubahnya.
